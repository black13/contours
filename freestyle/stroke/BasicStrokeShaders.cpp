...
namespace StrokeShaders {
  ...
  { 
	...
	++v)
      {
	real threshold = 0.1; //1e-3;  // was M_EPSILON

	if(!((fabs(v->x() -(previous)->x())<threshold) && ((fabs(v->y() - (previous)->y())<threshold))))
    ...
    // trying to debug screwy strokes
    it = stroke.strokeVerticesBegin();
    Vec2d lastPos(it->x(), it->y());
    ++it;
    int i=0;
    for(; it != stroke.strokeVerticesEnd(); ++it, ++i)
      {
	Vec2d pos(it->x(), it->y());
	real d=sqrt( (pos - lastPos) * (pos - lastPos));
	if ( d > 10)
	  {
	    printf("d = %f; originalSize = %d, newsize = %d; i = %d; ", d, originalSize, newsize, i);
	    printf("lastPos = %f %f, pos = %f %f\n", lastPos[0], lastPos[1], pos[0], pos[1]);
	    //	    assert(0);
	  }
	lastPos = pos;
      }
    ...
    */
    ...
    Vec2d lastPos(it->x(), it->y());
    ++it;
    for(; it != stroke.strokeVerticesEnd(); ++it)
      {
	Vec2d pos(it->x(), it->y());
	real d= (pos - lastPos) * (pos - lastPos);
	if ( d > 1)
	  {
	    printf("data.size() = %d; d = %f; ", data.size(), d);
	    printf("lastPos = %f %f, pos = %f %f\n", lastPos[0], lastPos[1], pos[0], pos[1]);
	    //	    assert(0);
	  }
	lastPos = pos;
