...
void Canvas::RenderLayer(const StrokeRenderer *iRenderer, int layer)
{
    if(layer<0 || layer>=_StyleModules.size() || !_Layers[layer])
        return;
    _Layers[layer]->Render(iRenderer);
}
