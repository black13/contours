...
struct DebugPoint
{
  typedef enum { ERROR, CUSP, RAY_TRACE_VISIBLE, RAY_TRACE_INVISIBLE, 
                 INVISIBLE_BACK_FACE, INVISIBLE_ONE_RING_OVERLAP,
		 INTERSECTION_2D, INTERSECTION_2D_ON_SURFACE,
		 PO_CROSSING_INSIDE_TRI, PO_CROSSING_ON_MESH_EDGE,
		 SI_SIL_CONNECTION, FRONT_FACING_VERTEX, BACK_FACING_VERTEX,
		 CONTOUR_VERTEX, ISOPHOTE} PointType;
  PointType pointType;
  Vec3r point3D;
  bool knownInvis;
  SVertex * sv;
...
public:
  ...
  //  typedef vector<VisRegion*> visregion_container;
...
private:
  ...
  vector<DebugPoint*> _debugPoints;
  map<WFace*,FacePOData*> _facePOData;  // maybe this should just be pointed to from the faces?
  multimap<int,InconsistentTri*> _inconsistentTris;
  vector<pair<WFace*,Vec3r> > _poCuspFaces;

  //  visregion_container _visRegions;
...
public:
  ...
  //  inline visregion_container & visRegions() { return _visRegions; }
  ...
  // split an fedge (and it's view edge) at a specified 3D point
  NonTVertex * CreateNonTVertex(Vec3r point3D, Vec3r point2D, FEdge * fe);
  ...
  void MergeTVertices(set<TVertex*> & tvGroup);
  void MergeNonTVertices(NonTVertex * ntv1, NonTVertex * ntv2);
  ...
  DebugPoint* addDebugPoint(DebugPoint::PointType ptType, Vec3r point3D, bool knownInvis=false, SVertex * sv=NULL);
  DebugPoint* addDebugPoint(DebugPoint::PointType ptType, Vec3r point3D, Vec3r pairPoint3D, 
		     bool knownInvis=false, SVertex * sv=NULL, bool pairKnownInvis=false, SVertex * pairSV=NULL);
  void addRIFDebugPoint(DebugPoint::PointType ptType, Vec3r point3D, char * debugString, real radialCurvature);

  void addCuspFace(WFace * f, Vec3r color) { _poCuspFaces.push_back(pair<WFace*,Vec3r>(f,color)); }

  FacePOData * facePOData(WFace * f) { assert(_facePOData.find(f) != _facePOData.end()); return _facePOData[f]; }
  void setFacePOData(WFace *f, FacePOData * data) { _facePOData[f] = data; }
  map<WFace*,FacePOData*> & facePOData() { return _facePOData; }

  void addInconsistentTri(InconsistentTri * tri, int regionIndex) 
  { _inconsistentTris.insert(pair<int,InconsistentTri*>(regionIndex, tri)); }

  //private:
  // variables for selection/picking
  //  map<ViewEdge*,int> _mapVEName;
  //  map<int,ViewEdge*> _mapNameVE;
  //  map<DebugPoint*,int> _mapPointName;
  //  map<int,DebugPoint*> _mapNamePoint;
};
...
public:
  ...
  inline bool sameFace() { return _sameFace; }
  ...
  void SetSameFace(bool sf) { _sameFace = sf; }
  ...
  void recreateEdgeList();

  // added by Aaron for debugging
  int numEdges() const { return _sortedEdges.size(); }
  directedViewEdge * getEdge(int i) {return _sortedEdges[i]; }
};
...
private:
  ...
  Vec3r _colorID; // for visualization purposes
...
public:
  ...
  inline ViewEdge() {
    ...
    _wasAmbiguous = false;
    _colorID = Vec3r(drand48() , drand48() , drand48() );
  }
...
protected:
  ...
  inline ViewEdge(ViewEdge& iBrother)
  {
    ...
    // debugging...
    // this function seems problematic because the superclass constructor isn't called (because the constructor isn't declared virtual), thereby adding this to the list of interface1ds
    assert(0); 
  }
  /*! Cloning method. */
...
public:
  ...
  inline void MarkInconsistent(bool v = true) { _inconsistentVisibility = v; }
  inline void MarkAmbiguous(bool v = true) { _ambiguousVisibility = v; _wasAmbiguous = v; }
  inline void FixAmbiguous() { _ambiguousVisibility = false;}

  int visVotes;
  int invisVotes;

  inline Vec3r colorID() const { return _colorID; }
  ...
  inline bool inconsistentVisibility() const { return _inconsistentVisibility; }
  inline bool ambiguousVisibility() const { return _ambiguousVisibility; }
  inline bool wasAmbiguous() const { return _wasAmbiguous; }
...
public:
  ...
  void SplitEdge(ViewVertex * newVertex, ViewEdge * vEdge, ViewEdge * & newViewEdge,bool inFront);
  //    inline void SplitEdgeAaron(FEdge *fe, 
  //                        const vector<TVertex*>& iViewVertices, 
  //                        vector<FEdge*>& ioNewEdges,
  //                        vector<ViewEdge*>& ioNewViewEdges);
  /* accessors */
  ...
  {
    // debugging: make sure we don't already have it
    for(vector<ViewEdge*>::iterator it = _Edges.begin(); it != _Edges.end(); ++it)
      {
	assert((*it) != iEdge);
      }
...
struct DebugVisOptions
{
    ...
    bool visibleOnly;
    bool showPunchouts;
    bool showPOCuspTesselation;
    bool showPairs;
    bool radialCurvaturePoints;
    bool normals;
    bool showViewVertices;
    bool nDotVshading;
    bool showWireframe;
    bool showInconsistentFaces;
    bool showRadial;
    bool lighting;

    typedef enum { EC_TYPE, EC_ID_COLOR } ecT;
    ecT edgeColors;
    
    bool showSpecificPoints;
    DebugPoint::PointType pointsToShow;
    
    bool limitRegion;
    int regionIndex;
    unsigned int selectionName;
};
